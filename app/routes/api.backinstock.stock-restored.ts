...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const baseUrl = shopCfg.shopBaseUrl.replace(/\/+$/, ""); // strip trailing slashes

  const productUrl = handle
    ? `${baseUrl}/products/${handle}`
    : `${baseUrl}/products/${productNumericId}`;
